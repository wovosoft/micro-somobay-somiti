    ...
    <DataTable
        ...
        <Column field="date" header="Date">
            <template #editor="item">
                <input-text v-model="item.data.date" type="date" class="p-inputtext-sm" style="width:100%;"/>
            </template>
        </Column>
        <Column field="type" header="Type">
            <template #body="item">
                {{ item.data.type ? expense_types.find(i => i.value === item.data.type).label : "" }}
            </template>
            <template #editor="item">
                <dropdown :options="expense_types"
                          option-label="label"
                          option-value="value"
        ...
        <Column field="description" header="Description">
            <template #editor="item">
                <input-text v-model="item.data.description" class="p-inputtext-sm" style="width:100%;"/>
            </template>
        </Column>
        <Column field="amount" header="Amount">
            <template #editor="item">
                <input-number v-model="item.data.amount" class="p-inputtext-sm" style="width:100%;"/>
            </template>
        </Column>
        ...
        <form ref="the_form" @submit.prevent="handleSubmit">
            <div class="p-fluid p-formgrid p-grid">
                ...
                <div class="p-field p-col-12 p-md-6 p-lg-3">
                    ...
                    <InputText v-model="item.description" :required="true" type="text"/>
                </div>
                <div class="p-field p-col-12 p-md-6 p-lg-3">
                    <label>Amount</label>
                    <InputNumber v-model="item.amount" :max-fraction-digits="2" :min="0"/>
                </div>
            </div>
        </form>
...
export default {
    ...
    methods: {
        exportItems() {
            ...
            form.setAttribute("action", "/admin/expenses/export");
        ...
        getItems(params = {}) {
            return axios.post("/admin/expenses", {...params, ...this.query, ...this.datatable}).then(res => {
        ...
        handleSubmit() {
            ...
            axios
                .post("/admin/expenses/store", JSON.parse(JSON.stringify(this.item)))
                .then(res => {
                    this.$toast.add({
                        severity: 'success',
                        summary: 'Success',
        ...
        trashItems() {
            if (this.selectedProducts.length && confirm("Are You Sure?")) {
                ...
                axios
                    .post("/admin/expenses/destroy", {id: ids})
                    .then(res => {
                        this.$toast.add({
                            severity: 'success',
                            summary: 'Success',
        ...
        rowEditSave(e) {
            axios
                .post("/admin/expenses/store", JSON.parse(JSON.stringify(e.data)))
                .then(res => {
                    this.$toast.add({
                        severity: 'success',
                        summary: 'Success',
